  ...
  return (
    <div className="bg-white text-slate-900 font-sans">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
          {steps.map((step) => (
            <div key={step.id} className="bg-white rounded-lg p-6">
              <div className="w-12 h-12 bg-[#A800FF] rounded-full flex items-center justify-center text-white font-semibold text-lg mb-4 select-none">
                {step.id}
              </div>
      ...
      <div className="flex justify-center mt-12 ">
        <button
          type="button"
          className="bg-[#A800FF] text-white font-sans font-semibold px-12 py-4 rounded-2xl hover:bg-[rgb(162,0,138)] transition-colors"
        >
          Apply Now
        </button>
      </div>
